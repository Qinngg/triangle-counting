...
#include <random>
#include<omp.h>
#include <algorithm> // 为了使用 std::shuffle
#include <stack>
#include <list>
#include <queue>
using namespace std;
...
float count_N_1(map<int, int> *a_mat, float roll, const set<int> &idx_set){
	int idx;
	float N_1 = 0.0;
	for(auto it1=idx_set.begin();it1!=idx_set.end();it1++)
	{
		int a=0,e=0;
		idx=*it1;
		for(auto it2=a_mat[*it1].begin();it2!=a_mat[*it1].end();it2++)
		{
			if(idx < (it2->first) && it2->second>0)	e++;
		}
		int i=0;
		#pragma omp parallel for reduction(+:a)
		for(i=0;i<NodeNum;i++)
		{
			if(idx > i && (idx_set.find(i)==idx_set.end()))	a+=1;
		}
		N_1+=(1.0)*a*e*roll*roll;
	}
	return N_1;
}

float count_N_2(map<int, int> *a_mat, float roll, const set<int> &idx_set){
	int idx;
	float N_2 = 0.0;
	float cnt1=0.0,cnt2=0.0;
	vector<int> vec; //记录每一个坏人的编号
	map<int,set<int>> m_cc;   //记录每一个坏人和谁之间有关联
	for(auto it1=idx_set.begin();it1!=idx_set.end();it1++)
	{
		int e=0;
		idx=*it1;
		...
		for(auto it2=a_mat[*it1].begin();it2!=a_mat[*it1].end();it2++)
		{
			if(it2->second>0)	m_cc[idx].insert(it2->first);
			if(idx < it2->first && it2->second>0)	e++;
		}
		cnt1+=e*(e-1)/2.0;
	}
	...
	int i,j;
	#pragma omp parallel for reduction(+:cnt2) private(j)
	for(i=0;i<vec.size()-1;i++)
	{
		for(j=i+1;j<vec.size();j++)
		{
			if((m_cc[vec[i]].find(vec[j]))==m_cc[vec[i]].end())	continue;
			for(auto it=m_cc[vec[i]].rbegin();it!=m_cc[vec[i]].rend();it++)
			{
				if(*it > vec[j])	cnt2+=1;
                else break;
			}
...
int count_N_3(map<int, int> *a_mat,const set<int> &idx_set){
	map<int, int> *a_attack;
	a_attack=new map<int,int> [NodeNum];
	for(int i=0;i<NodeNum;i++)
	{
		if(idx_set.find(i)!=idx_set.end())	a_attack[i]=a_mat[i];
	}
	int tri_num = 0;
	int i,j,k;
	#pragma omp parallel for reduction(+:tri_num) private(j,k)
	for(i=0;i<NodeNum;i++){
		//int j,k;
		for (auto aitr = a_attack[i].begin(); aitr != a_attack[i].end(); aitr++) {
			j = aitr->first;
			if (i >= j) continue;
			for (auto aitr2 = a_attack[i].begin(); aitr2 != a_attack[i].end(); aitr2++) {
				k = aitr2->first;
				if (j >= k) continue;
				if(a_attack[j].count(k) > 0) tri_num+=1;
			}
		}
	}
	delete [] a_attack;
...
void attack_no_max_degree(map<int, int> *a_mat,const set<int> &idx_set){
	int idx;
	for(auto it1=idx_set.begin();it1!=idx_set.end();it1++)
	{
		idx=*it1;
...
void attack_no_edge(map<int, int> *a_mat,const set<int> &idx_set){
	int idx;
	for(auto it1=idx_set.begin();it1!=idx_set.end();it1++)
	{
		idx=*it1;
...
void attack_max_degree(map<int, int> *a_mat, const set<int> &idx_set, const int &max_deg) {
    cout << "attack_max_degree" << endl;

    // 把idx_set转换成sorted vector
    vector<int> sorted_idx(idx_set.begin(), idx_set.end());
    sort(sorted_idx.begin(), sorted_idx.end(),greater<int>());

    // 将sorted_idx分成两组
    vector<int> first_group(sorted_idx.begin(), sorted_idx.begin() + min(max_deg, static_cast<int>(sorted_idx.size())));
    vector<int> second_group(sorted_idx.begin() + min(max_deg, static_cast<int>(sorted_idx.size())), sorted_idx.end());
    ...
    std::set_difference(full_set.begin(), full_set.end(), 
                        idx_set.begin(), idx_set.end(), 
                        std::inserter(remaining_set, remaining_set.begin()));

    // 将好人集合转换成sorted vector
    vector<int> sorted_idx_remaining(remaining_set.begin(), remaining_set.end());
    sort(sorted_idx_remaining.begin(), sorted_idx_remaining.end(),greater<int>());
    ...
    // first group
	vector<int> count(first_group.size(),0);
    for (int i=0;i<first_group.size();i++){
        if(count[i]>=max_deg) break;
        int idx=first_group[i];
        for (int j=i-1;j>=0;j--){
            if(count[j]>=max_deg)  continue;
            int idx2=first_group[j];
            a_mat[idx][idx2] = 1;
            a_mat[idx2][idx] = 1;
            count[i]+=1;
            count[j]+=1;
		...
		if(count[i]>=max_deg)  continue;

        int remaining = max_deg - count[i];

        int remaining_count = 0;
        for (int rem_idx : sorted_idx_remaining) {
            if (rem_idx < idx) {
                a_mat[idx][rem_idx] = 1;
                a_mat[rem_idx][idx] = 1;
                remaining_count++;
                if (remaining_count >= remaining) {
                    break;
                }
            }
        }
    ...
    //  second group 
    for (int idx_second : second_group) {
        for (int idx_first : first_group) {
            a_mat[idx_second][idx_first] = 1;
            a_mat[idx_first][idx_second] = 1;
        }
    }
}


void CalcNLocTri_attack_max_degree(map<int, int> *a_mat, int *deg, string outfile, double &tri_num_ns, int emp){
	...
	while(idx_set.size()<0.2*NodeNum)
	{
		idx_set.insert(rand()%NodeNum);
	}
	...
	float N_sub1,N_sub2,N_sub3;
	N_sub1=count_N_1(a_mat,roll,idx_set);
	cout<<"f1"<<endl;
	N_sub2=count_N_2(a_mat,roll,idx_set);
	cout<<"f2"<<endl;
	N_sub3=count_N_3(a_mat,idx_set);
	cout<<"f3"<<endl;
	float N_sub=0.0+N_sub1+N_sub2+N_sub3;

	attack_max_degree(a_mat,idx_set,max_deg);

	float N_plus1,N_plus2,N_plus3;
	N_plus1=count_N_1(a_mat,roll,idx_set);
	cout<<"f11"<<endl;
	N_plus2=count_N_2(a_mat,roll,idx_set);
	cout<<"f22"<<endl;
	N_plus3=count_N_3(a_mat,idx_set);
	cout<<"f33"<<endl;
	float N_plus=0.0+N_plus1+N_plus2+N_plus3;

	if(N_plus<=2*N_sub)
	{
		cout<<"attack_no_edge"<<endl;
		attack_no_edge(a_mat,idx_set);
	}
	...
	// 图投影
	for(i=0;i<NodeNum;i++){
		int node_degree = a_mat[i].size();

		if (node_degree > max_deg)
		{
			// 将当前节点的所有邻居放入一个vector中
			vector<int> neighbors;
			for(const auto& pair : a_mat[i])
			{
				neighbors.push_back(pair.first);
			}

			// 随机打乱这些邻居的顺序
			std::shuffle(neighbors.begin(), neighbors.end(), std::default_random_engine());

			// 删除超出的邻居，直到达到max_deg_ns
			for (int j = 0; j < node_degree - max_deg; j++)
			{
				a_mat[i].erase(neighbors[j]);
				a_mat[neighbors[j]].erase(i);
			}
		}
	}
	...
	// Degree --> deg_ns
	#pragma omp parallel for private(aitr)
	for(i=0;i<NodeNum;i++){
	...
	tot_edge_num_ns = 0;
	#pragma omp parallel for reduction(+:tot_edge_num_ns)
	...
	#pragma omp parallel for reduction(+:tri_num) private(j,k,aitr,aitr2)
	for(i=0;i<NodeNum;i++){
		for (aitr = a_mat_ns[i].begin(); aitr != a_mat_ns[i].end(); aitr++) {
			...
			for (aitr2 = a_mat_ns[i].begin(); aitr2 != a_mat_ns[i].end(); aitr2++) {
				k = aitr2->first;
				if (j >= k) continue;
				if(a_mat_ns[j].count(k) > 0) tri_num+=1;
			}
		}
	}
	cout<<"fq4"<<endl;
...
void attack_social_network(map<int, int> *a_mat, const set<int> &idx_set, const int &max_deg) {
    cout << "attack_social_network" << endl;

    // 将 idx_set 的节点复制到 new_set 中
	set<int> new_set;
    for (int idx : idx_set) {
        new_set.insert(idx);
    }

    // 添加小于 idx_set 中节点的所有节点到 new_set 中
    for (int idx : idx_set) {
        for (int i = idx+1; i < NodeNum; i++) {
            new_set.insert(i);
        }
	...
	// 生成节点度数
	map<int, int> degrees;
	for (int node : new_set) {
		int degree = 0;
		if (a_mat[node].find(node) != a_mat[node].end()) { // 如果节点在 a_mat 中存在
			for (const auto &neighbour_pair : a_mat[node]) {
				int neighbour = neighbour_pair.first;
				if (new_set.find(neighbour) != new_set.end()) { // 如果邻居也在 new_set 中
					degree++;
				}
			}
		}
    ...
    for (int s : new_set) {
        ...
        while (!Q.empty()) {
            ...
            if (a_mat->find(v) == a_mat->end()) continue;

            for (const auto &neighbor_pair : a_mat[v]) {
                int w = neighbor_pair.first;

                if (new_set.find(w) == new_set.end()) continue;  // 只考虑new_set中的节点
	...
	// 将度数和介数相加，形成分数
	map<int, double> T_values;
    for (int node : new_set) {
        T_values[node] = degrees[node] + betweenness[node];
    }

    // 将T值存入一个向量，并排序
    vector<pair<int, double>> sorted_T;
    for (const auto &entry : T_values) {
        sorted_T.push_back(entry);
    }

    sort(sorted_T.begin(), sorted_T.end(), [](const pair<int, double> &a, const pair<int, double> &b) {
        return a.second > b.second;  // 从大到小排序
    });

	// 攻击者与前max_deg分数高的节点连接
	// 获取前max_deg个节点
    vector<int> top_T_nodes;
    for (int i = 0; i < sorted_T.size(); i++) {
        top_T_nodes.push_back(sorted_T[i].first);
    }
    ...
    // 将idx_set中的所有节点与top_T_nodes中的节点连接
    for (int idx_node : idx_set) {
		int cnt = 0;
        for (int top_node : top_T_nodes) {
			if(idx_node<top_node){
				a_mat[idx_node][top_node] = 1;
            	a_mat[top_node][idx_node] = 1;
				cnt+=1;
				if(cnt>max_deg) break;
			}
        }
    }
}

void CalcNLocTri_attack_to_social_network(map<int, int> *a_mat, int *deg, string outfile, double &tri_num_ns, int emp){
	...
	attack_social_network(a_mat,idx_set,max_deg);
	...
	cout<<N_sub1<<" "<<N_sub2<<" "<<N_sub3<<endl;
	cout<<N_plus1<<" "<<N_plus2<<" "<<N_plus3<<endl;
	cout<<N_plus<<" "<<2*N_sub<<endl;
...
void CalcNLocTri_attack(map<int, int> *a_mat, int *deg, string outfile, double &tri_num_ns, int emp){
	...
	attack_no_max_degree(a_mat,idx_set);
...
void CalcNLocTri_no_attack(map<int, int> *a_mat, int *deg, string outfile, double &tri_num_ns, int emp){
	map<int, int> *a_mat_ns;			// noisy adjacency matrix
	map<int, int>::iterator aitr;
	map<int, int>::iterator aitr2;
	int *deg_ns;									// noisy degree
...
void attack_baseline_flip(map<int, int> *a_mat,const set<int> &idx_set){
	cout<<"attack_baseline_flip"<<endl;
	set<int> s[idx_set.size()];
    int idx;
	for(auto it1=idx_set.begin();it1!=idx_set.end();it1++)
	{
        idx=*it1;
...
void attack_baseline_flip_max_degree(map<int, int> *a_mat,const set<int> &idx_set, const int &max_deg){
	cout<<"attack_baseline_flip_max_degree"<<endl;
	set<int> s[idx_set.size()];
    int idx;
	for(auto it1=idx_set.begin();it1!=idx_set.end();it1++)
	{
	...
	{
		idx = *it1;
		int node_degree = a_mat[idx].size();

		if (node_degree > max_deg)
		{
			// 将当前节点的所有邻居放入一个vector中
			vector<int> neighbors;
			for(const auto& pair : a_mat[idx])
			{
				if(pair.second>0){
					neighbors.push_back(pair.first);
				}
			...
			// 删除超出的邻居，直到达到max_deg
			for (int i = 0; i < node_degree - max_deg; i++)
			{
				a_mat[idx].erase(neighbors[i]);
				a_mat[neighbors[i]].erase(idx);
			}
		}
	}
...
void attack_baseline_Gaussian(map<int, int> *a_mat,const set<int> &idx_set){
	cout<<"attack_baseline_Gaussian"<<endl;
	set<int> s[idx_set.size()];
    int idx;
	for(auto it1=idx_set.begin();it1!=idx_set.end();it1++)
	{
        ...
        for(int i=idx+1;i<NodeNum;i++)
        {
			...
			std::normal_distribution<> distribution(mean, stddev);   // 定义正态分布
...
void CalcNLocTri_baseline_flip_max_degree(map<int, int> *a_mat, int *deg, string outfile, double &tri_num_ns, int emp){
	...
	attack_baseline_flip_max_degree(a_mat,idx_set,max_deg);
...
void CalcNLocTri_baseline_flip(map<int, int> *a_mat, int *deg, string outfile, double &tri_num_ns, int emp){
	...
	attack_baseline_flip(a_mat,idx_set);
...
void CalcNLocTri_baseline_Gaussian(map<int, int> *a_mat, int *deg, string outfile, double &tri_num_ns, int emp){
	...
	attack_baseline_Gaussian(a_mat,idx_set);
...
void CalcILocTri_attack_max_degree(map<int, int> *a_mat, int *deg, string outfile, double &tri_num_ns, double &sen_tri){
...
void CalcILocTri_attack_to_social_network(map<int, int> *a_mat, int *deg, string outfile, double &tri_num_ns, double &sen_tri){
...
void CalcILocTri_attack(map<int, int> *a_mat, int *deg, string outfile, double &tri_num_ns, double &sen_tri){
...
void CalcILocTri_baseline_flip(map<int, int> *a_mat, int *deg, string outfile, double &tri_num_ns, double &sen_tri){
...
void CalcILocTri_baseline_flip_max_degree(map<int, int> *a_mat, int *deg, string outfile, double &tri_num_ns, double &sen_tri){
...
void CalcILocTri_baseline_Gaussian(map<int, int> *a_mat, int *deg, string outfile, double &tri_num_ns, double &sen_tri){
...
int main(int argc, char *argv[])
{
	...
	#pragma omp parallel
	{
		if(omp_get_thread_num()==0) cout<<omp_get_num_threads()<<"threads excing"<<endl;;
	}
	int all_node_num;
	int triplet_num;
	int **node_order;
		...
		for(i=0;i<NodeNum;i++){
			for (aitr = a_mat[i].begin(); aitr != a_mat[i].end(); aitr++) {
				...
				for (aitr2 = a_mat[i].begin(); aitr2 != a_mat[i].end(); aitr2++) {
					k = aitr2->first;
					if (j >= k) continue;
					if(a_mat[j].count(k) > 0) tri_num+=1;
				}
			}
		}
...
//			CalcNLocTri(a_mat, deg, outfile, tri_num_ns, 1);
			if(NodeNum <= 1000000) {
				if(attack == 0) CalcNLocTri_no_attack(a_mat, deg, outfile, tri_num_ns, 1);  //无攻击
				else if(attack == 1) {
					cout << "Attack With No Constraint on Node Degree" << endl;
					CalcNLocTri_attack(a_mat, deg, outfile, tri_num_ns, 1);  //有攻击(节点度无限制)
				}
				else if(attack == 2) {
					cout << "edge flip attack With No Constraint on Node Degree" << endl;      //边翻转（节点度无限制）
					CalcNLocTri_baseline_flip(a_mat, deg, outfile, tri_num_ns, 1); 
				} //baseline1
				else if(attack == 3) {
					cout << "Gaussian attack";
					CalcNLocTri_baseline_Gaussian(a_mat, deg, outfile, tri_num_ns, 1); 
				}
				else if(attack == 4){
					cout << "Attack With Constraint on Node Degree" << endl;
					// max_deg_ns = CalcNSMaxDeg(deg, max_deg, EpsNsMaxDeg, outfile);
					CalcNLocTri_attack_max_degree(a_mat, deg, outfile, tri_num_ns, 1);      //有攻击(节点度有限制)
				}
				else if(attack == 5){
					cout << "edge flip attack With Constraint on Node Degree" << endl;
					// max_deg_ns = CalcNSMaxDeg(deg, max_deg, EpsNsMaxDeg, outfile);
					CalcNLocTri_baseline_flip_max_degree(a_mat, deg, outfile, tri_num_ns, 1);       //边翻转（节点度有限制）
				}
				else if(attack == 6){
					cout << "Attack To Socail Network" << endl;
					CalcNLocTri_attack_to_social_network(a_mat, deg, outfile, tri_num_ns, 1);      //对social network的特定攻击
				}
				else break;
			}
			else tri_num_ns = 0.0;
...
//			CalcNLocTri(a_mat, deg, outfile, tri_num_ns, 0);
			if(NodeNum <= 1000000) CalcNLocTri_no_attack(a_mat, deg, outfile, tri_num_ns, 0);
			else tri_num_ns = 0.0;
			sen_tri = 0.0;
		}
		// Interactive (2-rounds) local
        	...
        	CalcNLocSt(st2_num, st3_num, deg, outfile, st2_num_ns, st3_num_ns, sen_st2, sen_st3);
        	// Calculate #triangles
			if(attack == 0) CalcILocTri(a_mat, deg, outfile, tri_num_ns, sen_tri);  //无攻击
			else if(attack == 1) {
				cout << "Attack With No Constraint on Node Degree" << endl;
				CalcILocTri_attack(a_mat, deg, outfile, tri_num_ns, sen_tri);  //有攻击（节点度无限制）
			}
			else if(attack == 2) {
				cout << "edge flip attack With No Constraint on Node Degree" << endl;      //边翻转（节点度无限制）
				CalcILocTri_baseline_flip(a_mat, deg, outfile, tri_num_ns, sen_tri); 
			} //baseline1
			else if(attack == 3) {
				cout << "Gaussian attack";
				CalcILocTri_baseline_Gaussian(a_mat, deg, outfile, tri_num_ns, sen_tri); 
			}
			else if(attack == 4){
				cout << "Attack With Constraint on Node Degree" << endl;
				CalcILocTri_attack_max_degree(a_mat, deg, outfile, tri_num_ns, sen_tri);      //有攻击(节点度有限制)
			}
			else if(attack == 5){
				cout << "edge flip attack With Constraint on Node Degree" << endl;
				CalcILocTri_baseline_flip_max_degree(a_mat, deg, outfile, tri_num_ns, sen_tri);        //边翻转（节点度有限制）
			}
			else if(attack == 6){
				cout << "Attack To Socail Network" << endl;
				CalcILocTri_attack_to_social_network(a_mat, deg, outfile, tri_num_ns, sen_tri);       //对social network的特定攻击
			}
			else break;
		}
